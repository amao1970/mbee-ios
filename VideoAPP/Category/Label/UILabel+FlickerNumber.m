...
- (NSNumberFormatter *)flickerNumberFormatter{
    ...
    NSNumberFormatter * NumberFormatter = objc_getAssociatedObject(self, &flickerNumberFormatterKey);
...
- (void)dd_setNumber:(NSNumber *)number formatter:(NSNumberFormatter *)formatter{
    ...
    [self dd_setNumber:number duration:3.0 format:nil numberFormatter:formatter attributes:nil];
}
...
//    endNumber = multiple > 0 ? [number floatValue] * multiple : [number intValue];
    endNumber = [number intValue];
    
    float rangeintkey = (endNumber * Frequency);
    NSLog(@"_____rangeintkey___%.2f___",rangeintkey);
    [userInfo setObject:@(2) forKey:MultipleKey];
    [userInfo setObject:@(endNumber) forKey:EndNumberKey];
    [userInfo setObject:@(rangeintkey) forKey:RangeIntegerKey];
    ...
    self.flickerNumberFormatter.numberStyle = NSNumberFormatterDecimalStyle;
...
- (void)flickerAnimation:(NSTimer *)timer{
    ...
    NSString *formatStr = timer.userInfo[FormatKey]?:(self.flickerNumberFormatter?@"%@":@"%.2f");
...
//    NSLog(@"+++multiple+++%d",multiple);
//    NSString *formatStr = timer.userInfo[FormatKey]?:(self.flickerNumberFormatter?@"%@":[NSString stringWithFormat:@"%%.%df",(int)log10(multiple)]);
    NSLog(@"______%@________",self.flickerNumber);
//    self.text = [NSString stringWithFormat:@"%.2f",[self.flickerNumber floatValue]];
   self.text = [self finalString:@([self.flickerNumber floatValue]) stringFormat:nil andFormatter:self.flickerNumberFormatter];
    if(timer.userInfo[AttributeKey]){
        [self attributedHandler:timer.userInfo[AttributeKey]];
    }
    if([self.flickerNumber intValue] >= [timer.userInfo[EndNumberKey] intValue]){
//        NSNumber *num = timer.userInfo[ResultNumberKey];
//        self.text = [NSString stringWithFormat:@"%.2f",[num doubleValue]];
        self.text = [self finalString:timer.userInfo[ResultNumberKey] stringFormat:nil andFormatter:self.flickerNumberFormatter];
        if(timer.userInfo[AttributeKey]){
            [self attributedHandler:timer.userInfo[AttributeKey]];
        }
        [timer invalidate];
...
- (NSString *)finalString:(NSNumber *)number stringFormat:(NSString *)formatStr andFormatter:(NSNumberFormatter *)formatter{
    ...
    if(formatter){
        finalString = [self stringFromNumber:number numberFormatter:nil];
    }else{
        NSAssert([formatStr rangeOfString:@"%@"].location == NSNotFound, @"string format type is not matched,please check your format type");
        finalString = [NSString stringWithFormat:@"%.2f",[number floatValue]];
    }
    NSLog(@"___%@___",finalString);
    return finalString;
}
