...
@interface SVCMovieTableViewController ()
...
@implementation SVCMovieTableViewController
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [cell setUpMovieModel:self.dataList[indexPath.row]];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.isSelectedCell == NO) {
        ...
        [SVCCommunityApi checkVIPWithParams:nil BlockSuccess:^(NSInteger code, NSString *msg, NSDictionary *JSON) {
            if (code == 0) {
                SVCMoviePlayerModel *model = self.dataList[indexPath.row];
