...
#import "SVCMoviesFileViewController.h"
...
@interface SVCMoviesFileViewController ()<SVCBannerProtocol , SDCycleScrollViewDelegate>
...
@implementation SVCMoviesFileViewController
...
- (void)viewDidLoad {
    ...
    [self addTopview];
    [self.moviesBannerView setbannerImage:@"VIP影院"];
    [self.storyBannerView setbannerImage:@"卫视直播"];
        ...
        self.isSelectedCell = YES;
            ...
            if (user.uid.length < 1 ||  token.length < 1) {
                ...
                [SVCCommunityApi checkVIPWithParams:nil BlockSuccess:^(NSInteger code, NSString *msg, NSDictionary *JSON) {
                    if (code == 0) {
                        if (tag == 1209) {
                            UIVIPMoviesViewController *vc = [[UIVIPMoviesViewController alloc] initWithUrl:nil  type:VIPMovies];
                            self.navigationController.hidesBottomBarWhenPushed = YES;
                            [self.navigationController pushViewController:vc animated:YES];
                        }else{
                        // 原创小说
                            UIVIPMoviesViewController *vc = [[UIVIPMoviesViewController alloc] initWithUrl:nil  type:TVLive];
                            self.navigationController.hidesBottomBarWhenPushed = YES;
                            [self.navigationController pushViewController:vc animated:YES];
                            }
                    }else if (code == -1){
