...
@interface JXPlayBackViewController ()<UICollectionViewDelegate, UICollectionViewDataSource,WSLWaterFlowLayoutDelegate>
{
...
@implementation JXPlayBackViewController
...
-(void)getNetData{
    WS(weakSelf);
    [SVCCommunityApi getReplayListWithParams:nil BlockSuccess:^(NSInteger code, NSString *msg, NSDictionary *JSON) {
        if (code == 0) {
...
-(void)setUpMainView{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, Nav_HEIGHT, self.view.frame.size.width, SCR_HIGHT - Nav_HEIGHT -Tabbar_HEIGHT - GetHeightByScreenHeigh(45)+44) collectionViewLayout:_flow];
...
- (void)refreshLayout{
    ...
    _flow.collectionView.frame = CGRectMake(0, 0, self.view.frame.size.width, SCR_HIGHT - Nav_HEIGHT - Tabbar_HEIGHT-GetHeightByScreenHeigh(45)+44);
...
- (CGSize)waterFlowLayout:(WSLWaterFlowLayout *)waterFlowLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    JXRecommendModel *model = [self.dataAry objectAtIndex:indexPath.row];
...
{
    JXRecommendModel *model = self.dataAry[indexPath.row];
